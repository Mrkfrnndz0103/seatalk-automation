...
Write-Host "Uploading '$EnvFile' to Doppler project='$Project' config='$Config'..."
& $doppler secrets upload $EnvFile --project $Project --config $Config

if ($LASTEXITCODE -ne 0) {
    Write-Error "Failed to upload secrets to Doppler."
    exit $LASTEXITCODE
}
